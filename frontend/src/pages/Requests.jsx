...
export function RequestList() {
  ...
  useEffect(() => {
    ...
    const query = filter ? `?status_filter=${encodeURIComponent(filter)}` : "";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-1">
        {FILTERS.map(([value, label]) => (
          <button
            key={value}
            onClick={() => setFilter(value)}
            className={`btn ${filter === value ? "btn-dark" : "btn-ghost"}`}
          >
            {label}
          </button>
        ))}
      ...
      {rows.length === 0 ? (
        <Card bodyClass="p-0">
          <Empty icon={FileText} title="No requests found">
            Submit a prior-authorization packet to populate this list.
          </Empty>
        </Card>
      ) : (
              ...
              <tbody>
                {rows.map((r) => (
                  <tr
                    key={r.id}
                    className="row-link"
                    onClick={() => navigate(`/hospital/requests/${r.id}`)}
                  >
                    <td className="td">
                      <div className="num text-2xs">{r.case_number}</div>
                      <div className="mt-1 text-2xs text-ink-3">
                        {r.provider_specialty}
                      </div>
                    </td>
                    ...
                    <td className="td">
                      ...
                      <div className="text-2xs text-ink-3">
                        {r.requested_treatment} · {r.disease_severity}
                      </div>
                    </td>
...
export function RequestDetail() {
  ...
  const canAppeal =
    data.decision === "DENIED" &&
    !(data.appeals || []).some((a) => a.status === "OPEN");
  ...
  const submitAppeal = async () => {
    ...
    try {
      await api.post(`/api/requests/${id}/appeal`, appeal);
  ...
  // FIX: this used to always render data.criteria.rationale, which is the
  // rules engine's ORIGINAL verdict from adjudication time and is never
  // updated afterward. Once a reviewer decides the case or an appeal is
  // resolved, data.decision/data.status move on but this text didn't --
  // so an overturned denial could show status=APPROVED (green) next to a
  // banner that still read "Denied on coverage grounds...". Once a human
  // has acted (decision_source === 'REVIEWER'), prefer reviewer_notes,
  // which review.py now writes on every reviewer decision and every
  // appeal resolution.
  const bannerText =
    data.decision_source === "REVIEWER" && data.reviewer_notes
      ? data.reviewer_notes
      : data.criteria?.rationale;
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold">{data.diagnosis}</h1>

          <p className="mt-1 text-[13px] text-ink-2">
            {data.requested_treatment} · {data.features?.dose_category} ·{" "}
            {data.features?.frequency} · {data.features?.route}
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-5">
          <Metric
            label="Policy fit"
            value={data.policy_fit_score?.toFixed(3) ?? "—"}
          />
          ...
          <Metric
            label="Processing"
            value={
              data.processing_ms != null ? `${data.processing_ms} ms` : "—"
            }
          />
        </div>
      </div>
      ...
      <Card eyebrow="Clinical documentation" title="Uploaded documents">
        {data.documents?.length ? (
          <div className="space-y-2">
            {data.documents.map((doc) => (
              <div
                ...
                className="flex items-center gap-3 rounded-md border border-rule bg-canvas px-3 py-3"
              >
                <FileText size={16} className="text-provider" />
                ...
                <div>
                  ...
                  <div className="text-2xs text-ink-3">
                    {doc.page_count} pages ·{" "}
                    {(doc.extraction_confidence * 100).toFixed(1)}% extraction
                    confidence
                  </div>
                </div>
              </div>
          ...
          <p className="text-[13px] text-ink-3">No document attached.</p>
        )}
      </Card>

      {canAppeal && (
        <Card eyebrow="Appeal" title="Challenge this denial">
          {error && (
            <div className="mb-4">
              <Alert>{error}</Alert>
            </div>
          ...
          <button
            onClick={submitAppeal}
            disabled={appealing || appeal.rationale.trim().length < 10}
            className="btn mt-4 bg-provider text-white border-provider hover:bg-provider-deep"
          >
            <Send size={14} />
            {appealing ? "Filing appeal…" : "File appeal"}
          </button>
        </Card>
      )}

      {data.appeals?.length > 0 && (
        <Card eyebrow="Appeal history" title="Appeals on this case">
          <div className="space-y-3">
            {data.appeals.map((a) => (
              <div
                key={a.id}
              ...
              >
                ...
                <p className="mt-2 text-[13px] text-ink-2">{a.rationale}</p>

                {a.outcome_notes && (
                  <p className="mt-2 text-[13px]">
                    <strong>Outcome:</strong> {a.outcome_notes}
                  </p>
                )}
              </div>
            ))}
      ...
      <Card eyebrow="Audit trail" title="Case history">
        {audit.length === 0 ? (
          <p className="text-[13px] text-ink-3">No audit events recorded.</p>
        ) : (
          <div className="divide-y divide-rule">
            {audit.map((event) => (
...
// FIX: this component previously took a single `request` prop and read
// request.patient_name / request.diagnosis / etc. But Review.jsx (the
// payer-side case page) calls it as
// <SubmittedValues features={data.features} documents={data.documents} />
// -- a different prop shape entirely. Since `request` was always undefined
// in that call, the component returned null and the "Submitted values"
// card silently disappeared from every payer review page. It also used
// CSS classes ("card-header", "card-body", "field-label", "field-value")
// that don't exist in index.css, so even a matching prop name would have
// rendered unstyled. This version matches the features/documents contract
// Review.jsx already relies on and uses the real design-system classes.
const HIDE = /^_/;
export function SubmittedValues({ features, documents }) {
  const entries = Object.entries(features || {}).filter(([k]) => !HIDE.test(k));
  return (
    <Card eyebrow="Case record" title="Submitted values" bodyClass="p-0">
      {documents?.length > 0 && (
          ...
          {documents.map((d) => (
            ...
            >
              ...
              <span className="num shrink-0 text-2xs text-ink-3">
                {d.page_count}p · {pct(d.extraction_confidence)} extracted
              </span>
            </div>
          ))}
        </div>
      ...
      <dl className="grid grid-cols-2 gap-x-6 gap-y-0 px-4 py-2">
        {entries.map(([k, v]) => (
          ...
          >
            ...
            <dd className="num shrink-0 text-2xs">{String(v)}</dd>
          </div>
        ))}
      </dl>
    </Card>
...
// FIX: same problem as SubmittedValues above. Review.jsx calls this as
// <AuditTrail events={audit} />, but the previous version expected a
// `request` prop and read request.audit_trail -- always undefined here,
// so this returned null and the payer-side audit trail card vanished too.
// This version takes `events` directly, matching what Review.jsx already
// passes, and reuses the same event shape RequestDetail's inline audit
// list above uses (action / actor_email / created_at), plus the detail
// payload the backend's log() helper writes.
export function AuditTrail({ events }) {
  return (
    <Card eyebrow="Compliance" title="Audit trail" bodyClass="p-0">
      {events.length === 0 ? (
